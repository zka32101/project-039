...
- [`app/`](./app) — Flutterアプリ本体。Aha Moment動線
  ...
  設定・ペイウォール画面（RevenueCat接続、未接続時はデモ購入へ自動フォールバック）を実装。
...
2. Flutterアプリの雛形・Aha Moment動線・投稿フロー（ペイントUI）・Firebase接続・
   設定/ペイウォール画面（RevenueCat接続）を実装
   （このセッションの実行環境にFlutter SDK・Firebase/RevenueCatへのネットワークアクセスが無いため、
...
- 本人確認基盤・Cloud Functions（経路探索・影スコアバッチ・モデレーション）実装へ進む
