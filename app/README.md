...
- [x] 投稿フロー（ペイントUI）: 種別選択（木陰/アーケード/雨よけ/夜の明るさ/人通りが少ない）
  → 指でなぞる → 道路区間へ自動スナップ → 確認 →
  [本人確認済みユーザーのみ] コメント入力可 → 投稿完了（確定演出）→
  反映モードにより「即時反映」「承認待ち」を表示分岐
  - 「人通りが少ない」（`SpotType.lowFootTraffic`）は環境要因としての体感安全度の投稿種別。
    日陰・雨よけの有無のような客観的な観測と異なり、投稿者の主観・偏見の影響を受けやすいため、
    地域のモデレーション設定に関わらず常に人力承認を必須とする荒らし対策を組み込んでいる
    （`SpotType.requiresManualReview`、`functions/README.md`「主観的な投稿種別への荒らし対策」参照）
...
- [x] 投稿の相互チェック（確認投票／通報）バックエンド: `shadeSpots`/`brightnessSpots`の
  `votes`フィールド（従来は初期化されるのみで未使用）を、確認投票＋通報の2つの意味を持つ
  投票として本格化した（`functions/index.js`の`voteSpot`、詳細な設計意図は
...
- [x] 投稿一覧画面「投稿を確認」（`views/spots/spots_list_view.dart`）: 承認済み投稿
  ...
  - 各行タップで、その投稿に付けられたコメントのみをボトムシートで表示できるようにした
    （`SpotCommentService.fetchForSpot(spotId)`。`spotComments`は元々`spotId`フィールドを
    持っていたため、地図上のタップ操作という新規UIインタラクションを追加しなくても
